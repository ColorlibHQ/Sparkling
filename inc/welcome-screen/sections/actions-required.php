...
<div class="feature-section action-required demo-import-boxed" id="plugin-filter">
	...
	if ( ! empty( $sparkling_required_actions ) ):
		...
		foreach ( $sparkling_required_actions as $sparkling_required_action_key => $sparkling_required_action_value ):
			$hidden = false;
			if ( @$sparkling_show_required_actions[ $sparkling_required_action_value['id'] ] === false ) {
				$hidden = true;
			}
			if ( @$sparkling_required_action_value['check'] ) {
				continue;
			}
			$nr_actions_required ++;
			if ( $hidden ) {
			...
			<div class="sparkling-action-required-box">
				...
				<?php else: ?>
					<span data-action="add" class="dashicons dashicons-hidden sparkling-required-action-button"
					      id="<?php echo esc_attr( $sparkling_required_action_value['id'] ); ?>"></span>
				<?php endif; ?>
				<h3><?php if ( ! empty( $sparkling_required_action_value['title'] ) ): echo $sparkling_required_action_value['title']; endif; ?></h3>
				<p>
					<?php if ( ! empty( $sparkling_required_action_value['description'] ) ): echo $sparkling_required_action_value['description']; endif; ?>
					<?php if ( ! empty( $sparkling_required_action_value['help'] ) ): echo '<br/>' . $sparkling_required_action_value['help']; endif; ?>
				</p>
				<?php
				if ( ! empty( $sparkling_required_action_value['plugin_slug'] ) ) {
					...
					?>
					<p class="plugin-card-<?php echo esc_attr( $sparkling_required_action_value['plugin_slug'] ) ?> action_button <?php echo ( $active['needs'] !== 'install' && $active['status'] ) ? 'active' : '' ?>">
